...
def upload_image(userID):
    if request.method == 'POST':
        ...
        now = datetime.now()
        query = "INSERT INTO face (userID, image, date_uploaded) VALUES ('"+str(userID)+"', '"+ filename +"', '"+str(now)+"')"
        cur = con.cursor()
        cur.execute(query)
        con.commit()
        cur.close()
...
#             now = datetime.now()
#             query = "INSERT INTO face (userID, image, date_uploaded) VALUES ('"+str(userID)+"', '"+ file.filename +"', '"+str(now)+"')"
#             cur = con.cursor()
#             cur.execute(query)
#             con.commit()
#             cur.close()
...
@cross_origin(origin='*',headers=['Content- Type','Authorization'])
def image():
    global lol, s, person, accuracy, refresh
    lol = lol + 1
    global awit
    modeldir = './model/20180402-114759.pb'
    classifier_filename = './class/classifier.pkl'
...
if __name__ == '__main__':
   ...
   global con
#    con = mysql.connector.connect(host = '16.163.120.31', database='new_recogu', user='root', password='Imjk@2022', auth_plugin='mysql_native_password')
